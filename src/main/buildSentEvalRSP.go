...
func BuildSentEvalRSP(c *Client, ssResObj map[string]interface{}) (map[string]interface{}, error) {
	...
	for i, detail_item := range details {
		...
		if score >= 3.5 {
				goodWordIndex = append(goodWordIndex, strconv.FormatInt(int64(i+1), 10))
			/*
				...
				if word_info != nil && dp_type == -1 {
					...
					fmt.Printf("mp3_url: %s, pcm_path %s\n", mp3_url,  c.request["pcm_path"].(string))
					if nil != err {
						continue
					}
	...
	finalResObj["goodWordIndex"] = goodWordIndex
